      ...
      <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b pb-4 mb-6">
        <div>
          <div class="flex items-center gap-2 mb-1">
            ...
            <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400">Disposal</span>
          </div>
        ...
        <button @click="navigateTo('/disposal')"
        ...
        <div class="space-y-5">
          ...
          <div class="border rounded-xl p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <div class="space-y-0.5 sm:col-span-2"><p class="text-xs text-muted-foreground">Alasan</p><p class="text-sm font-medium">{{ detail.alasan }}</p></div>
              <div class="space-y-0.5"><p class="text-xs text-muted-foreground">No. Berita Acara</p><p class="text-sm">{{ detail.noBeritaAcara || '—' }}</p></div>
              <div class="space-y-0.5"><p class="text-xs text-muted-foreground">Nilai Sisa</p><p class="text-sm font-medium tabular-nums">{{ detail.nilaiSisa > 0 ? formatRupiah(detail.nilaiSisa) : '—' }}</p></div>
          ...
          <div class="border rounded-xl p-6 space-y-4">
            <h2 class="text-xs font-semibold text-muted-foreground uppercase tracking-widest">Dampak Finansial</h2>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div class="border rounded-lg p-4 space-y-1">
                <p class="text-xs text-muted-foreground">Nilai Sisa</p>
                <p class="text-sm font-bold tabular-nums">{{ formatRupiah(detail.nilaiSisa) }}</p>
              </div>
              <div class="border border-destructive/30 rounded-lg p-4 space-y-1 bg-destructive/5">
                <p class="text-xs text-muted-foreground">Kerugian Disposal</p>
                <p class="text-sm font-bold tabular-nums text-destructive">
                  {{ formatRupiah(Math.max(0, aset.hargaPerolehan - detail.nilaiSisa)) }}
                </p>
              </div>
            </div>
          </div>
...
import { transaksiService, type Transaksi, type TrxDisposal } from '@/services/transaksiService'
...
const detail    = ref<TrxDisposal | null>(null)
