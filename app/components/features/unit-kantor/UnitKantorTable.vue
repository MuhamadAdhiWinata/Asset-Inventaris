<!-- components/features/unit-kantor/UnitKantorTable.vue -->
<template>
  <div>
    ...
    <div class="rounded-lg border overflow-hidden">
      ...
      <div v-else-if="!loading && items.length === 0" class="flex flex-col items-center justify-center py-16 gap-2 text-muted-foreground">
        <Building2Icon class="h-8 w-8 opacity-40" />
        <p class="text-sm">Tidak ada unit kantor ditemukan</p>
      </div>
          ...
          <table class="w-full caption-bottom text-sm">
            <thead class="border-b bg-muted/40">
              <tr>
                ...
                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Nama Unit</th>
                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground hidden lg:table-cell">Telepon</th>
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-4 py-3 align-middle">
                  ...
                  <p class="text-xs text-muted-foreground lg:hidden mt-0.5">{{ item.telepon }}</p>
                </td>
                <td class="px-4 py-3 align-middle text-sm text-muted-foreground hidden lg:table-cell">{{ item.telepon }}</td>
                ...
                <td class="px-4 py-3 align-middle">
                  <span
                    ...
                    :class="STATUS_UNIT_CONFIG[item.status].class"
                  >
                    {{ STATUS_UNIT_CONFIG[item.status].label }}
                  </span>
                </td>
                <td class="px-4 py-3 align-middle">
        ...
        <div class="md:hidden divide-y">
          <div v-for="item in items" :key="item.id" class="p-4 hover:bg-muted/30 transition-colors">
            ...
            <div class="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
              <span class="text-muted-foreground">Telepon</span><span>{{ item.telepon }}</span>
              <span class="text-muted-foreground">Email</span><span class="truncate">{{ item.email }}</span>
              <span class="text-muted-foreground">Alamat</span><span class="truncate">{{ item.alamat }}</span>
              <span class="text-muted-foreground">Status</span>
              <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium w-fit" :class="STATUS_UNIT_CONFIG[item.status].class">
                {{ item.status }}
              </span>
            </div>
          </div>
    ...
    <div v-if="!loading && !error && totalItems > 0" class="flex items-center justify-between px-1 py-4">
      <p class="text-sm text-muted-foreground">
        Menampilkan {{ startItem }}–{{ endItem }} dari {{ totalItems }} unit kantor
      </p>
...
import {
  SearchIcon, Loader2Icon, RefreshCwIcon, Building2Icon,
  PencilIcon, Trash2Icon, ChevronLeftIcon, ChevronRightIcon,
} from 'lucide-vue-next'
import { type UnitKantor, STATUS_UNIT_CONFIG } from '@/services/unitKantorService'
...
const emit = defineEmits<{
  edit:            [item: UnitKantor]
  delete:          [item: UnitKantor]
