<!-- components/features/aset/AsetTable.vue -->
<template>
  <div>
    ...
    <div class="rounded-lg border overflow-hidden">
      ...
      <div v-else-if="!loading && items.length === 0" class="flex flex-col items-center justify-center py-16 gap-2 text-muted-foreground">
        <BoxIcon class="h-8 w-8 opacity-40" />
          ...
          <table class="w-full caption-bottom text-sm">
            <thead class="border-b bg-muted/40">
              <tr>
                ...
                <!-- Kolom unit hanya muncul untuk admin pusat (semua unit) -->
                ...
                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground hidden lg:table-cell">Kategori</th>
              ...
              >
                <td class="px-4 py-3 align-middle">
                  <div class="flex items-center gap-2">
                    <span class="font-mono text-xs font-medium">{{ item.kodeAset }}</span>
                    <!-- Badge initial balance -->
                    <span v-if="item.transaksiId === null"
                      class="inline-flex items-center rounded-full px-1.5 py-0.5 text-[10px] font-medium bg-muted text-muted-foreground">
                      IB
                    </span>
                  </div>
                </td>
                <td class="px-4 py-3 align-middle">
                  <p class="font-medium leading-snug">{{ item.barangNama }}</p>
                  <p class="text-xs text-muted-foreground lg:hidden mt-0.5">{{ item.kategoriNama }}</p>
                </td>
                ...
                <td class="px-4 py-3 align-middle text-right tabular-nums font-medium whitespace-nowrap hidden xl:table-cell"
                  :class="item.nilaiBuku === 0 ? 'text-muted-foreground' : ''">
                ...
                <td class="px-4 py-3 align-middle">
                  <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium whitespace-nowrap"
                    :class="STATUS_ASET_CONFIG[item.status].class">
                    {{ STATUS_ASET_CONFIG[item.status].label }}
                  </span>
                </td>
                <td class="px-4 py-3 align-middle">
                  <div class="flex items-center justify-center gap-1">
                    ...
                    <button
                      v-if="item.status === 'Aktif' || item.status === 'Dalam Perbaikan'"
                    ...
                    <button
                      ...
                      title="Hapus (hanya initial balance)">
          ...
          <div v-for="item in items" :key="item.id" class="p-4 hover:bg-muted/30 transition-colors">
            <div class="flex items-start justify-between gap-2 mb-3">
              <div>
                ...
                <p class="font-medium text-sm mt-0.5">{{ item.barangNama }}</p>
                <p v-if="showUnitColumn" class="text-xs text-muted-foreground mt-0.5">{{ item.unitNama }}</p>
              </div>
              <div class="flex gap-1 flex-shrink-0">
                ...
                <button v-if="item.status === 'Aktif'" @click="emit('edit', item)" class="h-8 w-8 inline-flex items-center justify-center rounded-md hover:bg-accent transition-colors">
            ...
            <div class="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
              ...
              <span class="font-medium tabular-nums" :class="item.nilaiBuku === 0 ? 'text-muted-foreground' : ''">{{ formatRupiah(item.nilaiBuku) }}</span>
              <span class="text-muted-foreground">Tgl. Perolehan</span>
              <span>{{ formatTanggal(item.tanggalPerolehan) }}</span>
              <span class="text-muted-foreground">Status</span>
              <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium w-fit" :class="STATUS_ASET_CONFIG[item.status].class">{{ item.status }}</span>
            </div>
          </div>
        </div>
      </template>
...
import {
  SearchIcon, Loader2Icon, RefreshCwIcon, BoxIcon,
  EyeIcon, PencilIcon, Trash2Icon, ChevronLeftIcon, ChevronRightIcon,
} from 'lucide-vue-next'
import { type Aset, STATUS_ASET_CONFIG, formatRupiah, formatTanggal } from '@/services/asetService'
import { type Kategori } from '@/services/kategoriService'
...
interface Props {
  ...
  showUnitColumn?: boolean  // true jika admin pusat (semua unit tampil)
}
